...
if not ADMIN_PASSWORD:
    logger.warning("ADMIN_PASSWORD environment variable is not set. Log download endpoint will not be secure.")
...
try:
    file_handler = logging.handlers.RotatingFileHandler(
        APP_LOG_FILE, maxBytes=1*1024*1024, backupCount=5, encoding='utf-8'  # 1MB per file, 5 backups
    )
...
api_router.include_router(
    devices.router,
    dependencies=[Depends(get_dependencies)]
)
api_router.include_router(
    helium_uplink.router,
    dependencies=[Depends(get_dependencies)]
)
api_router.include_router(
    trashbin.router,
...
api_router.include_router(
    trashbin_data.router,
    dependencies=[Depends(get_dependencies)]
)
api_router.include_router(
    admin.router,
    dependencies=[Depends(get_dependencies)] # Ensures DB dependencies are available if needed by admin routes
)
