...
# Add the parent directory to sys.path to allow imports from the models package
...
config.set_section_option(section, "POSTGRES_DB", os.getenv("POSTGRES_DB", "postgres"))

# Override sqlalchemy.url directly if needed
url = f"postgresql://{os.getenv('POSTGRES_USER', 'postgres')}:{os.getenv('POSTGRES_PASSWORD', 'postgres')}@{os.getenv('POSTGRES_HOST', 'localhost')}:{os.getenv('POSTGRES_PORT', '5432')}/{os.getenv('POSTGRES_DB', 'postgres')}"
