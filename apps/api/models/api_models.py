...
class DeviceBase(BaseModel):
    ...
    trashbin_id: Optional[str] = Field(
        None, description="DB ID of Trashbin the Device is assigned to"
    )
...
class DeviceListItem(DeviceBase):
    battery_level: Optional[int] = Field(
        None, description="Current battery level (0-100)"
    )
    last_seen: Optional[str] = Field(
        None, description="ISO formatted timestamp of last communication"
    )
    latest_data_id: Optional[int] = Field(
        None, description="DB ID of latest device data"
    )
...
class DeviceUpdate(DeviceBase):
    ...
    deviceProfileName: Optional[str] = Field(
        None, description="Device Profile Name of the device"
    )
    battery_level: Optional[int] = Field(
        None, ge=0, le=100, description="Current battery level (0-100)"
    )
    last_seen: Optional[str] = Field(
        None, description="ISO formatted timestamp of last communication"
    )
...
class TrashbinListItem(BaseModel):
    ...
    status: Optional[str] = Field(
        None, description="Status like active, maintenance, ..."
    )
    latitude: float = Field(..., description="Latitude of the trashbin")
    longitude: float = Field(..., description="Longitude of the trashbin")
    has_device: bool = Field(
        ..., description="Boolean whether trashbin has a sensor assigned to it"
    )
...
class TrashbinBase(BaseModel):
    ...
    location: str = Field(..., description="Location of trashbin")
    status: Optional[str] = Field(
        ..., description="Status like active, maintenance, ..."
    )
