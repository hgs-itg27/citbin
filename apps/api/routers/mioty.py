...
router = APIRouter(prefix="/mioty", tags=["Mioty"])


@router.post("/uplink", summary="Receive JSON Uplink Data from Mioty")
async def uplink(
    payload: Dict[str, Any] = Body(...), deps: Dict = Depends(get_dependencies)
):
    """
