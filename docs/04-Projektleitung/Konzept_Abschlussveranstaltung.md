# Konzept Abschlussveranstaltung
(bearbeitet am 12.06.2026, besprochen mit Frau Bieler)

## Termin
Donnerstag, 16. Juli 2026
10:00-13:00 Uhr
(Bürgermeister kommt von 12:00-13:00 Uhr dazu)

## Gästeliste

Das sind alle eingeladenen Gäste. Wer endgültig alles kommt, klären wir noch ab, und leiten es Frau Bieler weiter.
Frau Bieler gibt Informationen an Bürgermeister, Ihrer Vertretung (evtl. auch Frau Weißmann), technische Dienste, Presse weiter.

1. Intern:
- Herr Fehrenbach (stefan.fehrenbach@hgs-singen.de
- Herr Mümo (oliver.mueller-molenar@hgs-singen.de)
- Herr Laber (karl.laber@hgs-singen.de)
- Herr Martin (stefan.martin@hgs-singen.de)
- Herr Gantner (GMT) (ga@hgs-singen.online)
- Herr Zschoernig (GMT) (zs@hgs-singen.online)
- Herr Gripentrog (IT nächstes Jahr) (gp@hgs-singen.online)
- Herr Benz (IT nächstes Jahr) (bz@hgs-singen.online)
- Klasse TG 12/3 (so.jaeger@hgs-singen.online; d.saracoglu@hgs-singen.online)
- Samuel Brandao-Santana (TG13/3) (s.brandao@hgs-singen.online)
- Nicolas Wagner (TG13/3) (n.wagner@hgs-singen.online)

2. Extern:
- Frau Bieler (samira.bieler@singen.de)
- Oberbürgermeister (Bernd Häusler)
- Bürgermeister (Marcus Röwer) --> Ersatz für OB, falls dieser nicht kommen könnte
- Singen Aktiv (Vertreter: Wilfried Trah, Claudia Kessler-Franzen) (singen-aktiv@singen.de)
- Presse
- Herr Kahlig (ifm) (alexander.kahlig@ifm.com)
- Herr Bucher (ifm; Director I4.0 Systems and Applications) (oliver.bucher@ifm.com)
- Frank Watzlawik (ifm; Hochschulmarketing) (frank.watzlawik@ifm.com)
- Stephan Strittmatter (Sybit) (stephan.strittmatter@sybit.de)
- André Sommer (Sybit) (andre.sommer@sybit.de)
- Vertreter Stadtwerke/ technische Dienste

## Ablauf:
1. Ansprache von Herr Fehrenbach
2. Eventuell Videobotschaft/ Live-Zuschaltung von Frau Bieler
3. Präsentation vom Produkt durch die einzelnen Gruppen der Klasse (allgemein, Fortschritt, Ergebnis, Simulierung Mülleimer und Empfang der Daten)
4. Podiumsdiskussion Thema: Ausblick für die Zukunft (Jeweils ein Vertreter für Stadt, Schule, ifm, Sybit) (im Voraus absprechen was gesagt wird)
5. Begrüßung an den Bürgermeister Bernd Häusler, Ihm nochmal kurz eine Einführung in das Thema geben/ aktuellen Stand erzählen
6. Ansprache vom Bürgermeister Bernd Häusler (wenn er das möchte)
7. Stände anschauen, freie Gespräche etc.
8. Offenes Ende mit Buffet (organisiert Frau Bieler)

## Material:
Frau Bieler organisiert:
- Stühle (ca. 25-30 Stück)
- 1 Rednerpult
- 2 Stehtische für Podiumsdiskussion
- 4 normale Tische für die einzelnen Gruppenstände
- Pinnwände oder Wand zum Befestigen der Plakate, Bilder etc.
- Pinnnadeln, Klebeband
- Biemer/ Leinwand/ Technik, um privaten Laptop anzuschließen
- Hausmeister ab 9 Uhr zur Hilfe
- Essen und Getränke für Buffet (spätestens eine Woche davor, braucht Frau Bieler die genaue Anzahl der Gäste)
- Zusätzliche Tische für das Buffet

- Fau Bieler leitet Einladung/ Termin an OB, Presse, technische Dienste weiter

Schule bringt:
- Inhalt der Stände, auch Mülleimer und Sensoren, Benutzerhandbuch, Plakate, Flyer etc. zum Vorzeigen

## Nächste Schritte:
- Endgültige Einladung verschicken
